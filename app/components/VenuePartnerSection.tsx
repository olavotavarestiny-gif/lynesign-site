      ...
      <div className="max-w-5xl mx-auto px-4 sm:px-6 lg:px-8 relative z-10">
        <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} variants={containerVariants} className="text-center">
          <motion.div variants={itemVariants} className="mb-6">
            <span className="inline-block px-3 py-1 rounded-full bg-purple-100 text-purple-700 text-xs font-semibold uppercase tracking-wider">
              Partnership Opportunity
            </span>
          </motion.div>

          <motion.h2 variants={itemVariants} className="text-4xl md:text-5xl lg:text-6xl font-heading font-bold text-text-dark mb-6 leading-tight">
            Join Houston's Premier Indoor Advertising Network
          </motion.h2>

          <motion.p variants={itemVariants} className="text-xl text-text-light mb-12 max-w-3xl mx-auto leading-relaxed">
            ...
            {benefits.map((benefit, idx) => {
              ...
              return (
                <motion.div key={idx} variants={itemVariants} className="bg-white rounded-2xl p-8 shadow-lg hover:shadow-2xl transition-all duration-300 group">
                  <div className="inline-flex items-center justify-center w-14 h-14 rounded-xl bg-gradient-to-br from-accent-purple to-accent-coral mb-6 group-hover:scale-110 transition-transform">
                    <Icon className="w-7 h-7 text-white" />
                  </div>
                  <h3 className="text-xl font-heading font-bold text-text-dark mb-3">{benefit.title}</h3>
          ...
          <motion.button
            ...
            whileTap={{ scale: 0.95 }}
            className="inline-flex items-center gap-2 px-10 py-4 bg-gradient-to-r from-accent-coral to-accent-purple text-white rounded-xl font-bold text-lg shadow-xl hover:shadow-2xl transition-all duration-300 group"
          >
            Become a Partner
          ...
          <motion.div variants={itemVariants} className="mt-16 pt-12 border-t border-gray-200">
            ...
            <p className="font-semibold text-text-dark">— Sarah Mitchell, Restaurant Owner</p>
          </motion.div>
        </motion.div>
      </div>
    </section>
